...
%     - options: structure containing optional arguments
%         Possible fields (default values):
%         - dimB (2): 1/2 block size
%         - dimR (5): 1/2 search window size
% Outputs:
...
%
    ...
    % Nested function to compute distance between blocks
    function d = getDistance(subR, subI)
        % Mean Squared Sum of Differences (MSSD)
        d = sum((subR(:) - subI(:)).^2);
    end
